Original link: https://docs.liara.ir/dbaas/mysql/create-user/
...
دسترسی پیش‌فرض یا کاربر `root` در MySQL، یک اکانت مدیریتی با بیشترین سطح دسترسی است. این کاربر معادل administrator در سیستم‌های عامل مختلف است و می‌تواند تمام عملیات‌های مدیریتی و اجرایی در پایگاه داده را انجام دهد.
...
فقط داده‌ها را، در تمامی جداول موجود در دیتابیس مورد نظر، `SELECT` کند (بخواند). به عنوان مثال:
...
> همچنین بخوانید: [مستندات رسمی ایجاد کاربر در MySQL](https://dev.mysql.com/doc/refman/8.4/en/create-user.html)
