Original link: https://docs.liara.ir/ai/getting-started/nextjs-app-router/
...
برای شروع، یک برنامه‌ی جدید [NextJS](https://https://docs.liara.ir/paas/nextjs/getting-started/)، ایجاد کنید. دستور زیر یک دایرکتوری جدید به نام `my-ai-app` ایجاد می‌کند و درون آن، یک پروژه پایه NextJS می‌سازد:

```bash
npx create-next-app@latest my-ai-app
```

> هنگامی که از شما سوال شد، حتماً گزینه‌های `TypeScript` , `App Router` و `Tailwind CSS` را روی `yes` تنظیم کنید.
...
با اجرای دستور زیر (در لینوکس)، یک فایل `env.local.` در مسیر اصلی پروژه ایجاد کنید تا درون آن `baseUrl` هوش مصنوعی و [کلید API لیارا](https://https://docs.liara.ir/references/api/about/#api-access-key) را، قرار دهید:
...
در فایل  `app/page.tsx`، قطعه کد زیر را قرار دهید تا یک لیست از پیام‌های چت، نمایش داده شود و امکان ارسال پیام کاربر، فراهم گردد:
...
> حتماً دستور `'use client'` را به ابتدای فایل خود اضافه کنید. این کار به شما اجازه می‌دهد تا با استفاده از جاوااسکریپت، قابلیت‌های  interactive را به صفحه اضافه کنید.
...
برای به‌روزرسانی رابط کاربری و نمایش `tool invocation`، باید فایل `app/page.tsx` را تغییر دهید تا پیام‌هایی که شامل Tool هستند نیز، به درستی نمایش داده شوند.
...
فایل `app/page.tsx` خود را ویرایش کرده تا گزینه‌ی `maxSteps` را به آن، اضافه کنید:
...
> پروژه نهایی مورد بررسی در این آموزش، در [گیت‌هاب لیارا](https://github.com/liara-cloud/ai-sdk-examples/tree/master/NextJS-App-Router-ChatBot) موجود است که می‌توانید از آن، استفاده کنید.
