Original link: https://docs.liara.ir/ai/getting-started/nuxt/

# کار با AI SDK در برنامه‌های VueJS (Nuxt)
...
برای شروع، یک برنامه‌ی جدید [Nuxt](https://docs.liara.ir/paas/nodejs/related-apps/nuxtjs/)، ایجاد کنید. دستور زیر یک دایرکتوری جدید به نام `my-ai-app` ایجاد می‌کند و درون آن، یک پروژه پایه Nuxt می‌سازد:
...
در قطعه کد فوق، به‌جای `xxxxxxxxx`، مقادیر واقعی خود را، قرار دهید؛ در ادامه، متغیرهای تعریف‌شده  
را مانند قطعه کد زیر در فایل `nuxt.config.ts`، وارد کنید:
...
در مسیر `server/api/chat.ts`، یک API Route بسازید و قطعه کد زیر را درون آن، قرار دهید:
...
۱. یک Event Handler تعریف شده است که پیام‌ها را از بدنه‌ی request استخراج می‌کند. متغیر `messages` شامل سابقه‌ی گفتگو میان شما و چت‌بات است و context لازم را برای چت‌بات فراهم می‌کند تا پاسخ بعدی را تولید کند.
...
حالا که یک API route دارید که می‌تواند با یک LLM ارتباط برقرار کند، زمان آن رسیده که رابط کاربری (frontend) خود را راه‌اندازی کنید. پکیج UI در AI SDK، پیچیدگی‌های مربوط به رابط چت را در یک هوک به نام `useChat` ساده کرده است.  
در فایل  `pages/index.vue`، قطعه کد زیر را قرار دهید تا یک لیست از پیام‌های چت، نمایش داده شود و امکان ارسال پیام کاربر، فراهم گردد:
...
> اگر پروژه شما شامل `app.vue` به‌جای `pages/index.vue` است، تنها کافیست تا فایل `app.vue` را پاک کنید و یک `pages/index.vue` جدید با قطعه کد فوق ایجاد کنید.

این صفحه از هوک `useChat` استفاده می‌کند که به‌صورت پیش‌فرض، از API Route که قبل‌تر ایجاد کرده بودید، استفاده خواهد کرد.  
...
![work with chatbot in VueJS (Nuxt)](https://media.liara.ir/ai/ai-sdk/nuxt/working-with-chat-bot.png)
...
مانند قطعه کد زیر، به فایل `server/api/chat.ts` خود، Tool مربوط به وضعیت آب‌وهوا را اضافه کنید:
...
برای به‌روزرسانی رابط کاربری و نمایش `tool invocation`، باید فایل `pages/index.vue` را تغییر دهید تا پیام‌هایی که شامل Tool هستند نیز، به درستی نمایش داده شوند.  
...
![work with chatbot in vuejs (nuxt) with tools](https://media.liara.ir/ai/ai-sdk/nuxt/working-with-tools-in-chatbots.png)
...
فایل `pages/index.vue` خود را ویرایش کرده تا گزینه‌ی `maxSteps` را به آن، اضافه کنید:
...
![work with chatbot in vuejs (nuxt) with tools and maxsteps](https://media.liara.ir/ai/ai-sdk/nuxt/chatbots-tools-and-maxsteps.png)
...
Tool جدید برای تبدیل دما از فارنهایت به سلسیوس را به فایل `server/api/chat.ts` خود، مانند شکل زیر، اضافه کنید:
...
![work with chatbot in nuxt with two tools and maxsteps on 5](https://media.liara.ir/ai/ai-sdk/nuxt/chatbot-with-two-tools-and-maxsteps.png)
...
> پروژه نهایی مورد بررسی در این آموزش، در [گیت‌هاب لیارا](https://github.com/liara-cloud/ai-sdk-examples/tree/master/Vue-Nuxt-ChatBot) موجود است که می‌توانید از آن، استفاده کنید.
