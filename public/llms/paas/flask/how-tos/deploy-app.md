Original link: https://docs.liara.ir/paas/flask/how-tos/deploy-app/
...
قطعه کد زیر، یک نمونه از فایل `requirements.txt` برای برنامه‌های Flask است:
...
پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام `liaraignore.` یا `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری‌های venv. و dist به همراه محتوای آن‌ها نیست؛ چرا که لیارا در حین استقرار برنامه، آن‌ها را برای شما می‌سازد؛ پس بایستی اسامی این دایرکتوری‌ها در یکی از دو فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های Flask است که می‌توانید از آن، استفاده کنید:

[#### نمونه فایل `gitignore.` برای برنامه‌های Flask](https://github.com/liara-cloud/gitignore-templates/blob/master/flask/.gitignore)
...
در فیلد `platform` باید نوع پلتفرم خود را که flask است، مشخص کنید.
...
## تعیین ماژول اصلی برنامه

درصورتی که از نام دیگری به‌جز `app` یا `app.py` به‌عنوان نام ماژول اصلی برنامه‌تان استفاده کرده باشید نیاز هست  
در فایل `liara.json` در فیلدی به نام `appModule`، نام آن را مشخص کنید، به عنوان مثال، اگر نام ماژول server باشد، باید قطعه کد زیر را، در فایل مذکور، وارد کنید:
...
پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری‌های venv. و dist به همراه محتوای آن‌ها نیست؛ چرا که لیارا در حین استقرار برنامه، آن‌ها را برای شما می‌سازد؛ پس بایستی اسامی این دایرکتوری‌ها در فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های Flask است که می‌توانید از آن، استفاده کنید:
