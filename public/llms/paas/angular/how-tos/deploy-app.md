Original link: https://docs.liara.ir/paas/angular/how-tos/deploy-app/
...
قطعه کد زیر یک نمونه استاندارد، یک نمونه از فایل `package.json` استاندارد برای برنامه‌های Angular است:
...
پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام `liaraignore.` یا `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری node_modules به همراه محتوای آن نیست؛ چرا که لیارا در حین استقرار برنامه، آن را برای شما می‌سازد؛ پس بایستی نام این دایرکتوری در یکی از دو فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های Angular است که می‌توانید از آن، استفاده کنید:

[#### نمونه فایل `gitignore.` برای برنامه‌های Angular](https://github.com/liara-cloud/gitignore-templates/blob/master/angular/.gitignore)
...
در فیلد `platform` باید نوع پلتفرم خود را که Angular است، مشخص کنید.
...
Source Map در پلتفرم Angular لیارا به‌صورت پیش‌فرض غیرفعال است اما درصورت نیاز به فعال کردن این قابلیت می‌توانید قطعه‌کد زیر را در فایل `liara.json` قرار دهید:
...
پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام   `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری node_modules به همراه محتوای آن نیست؛ چرا که لیارا در حین استقرار برنامه، آن را برای شما می‌سازد؛ پس بایستی نام این دایرکتوری در فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های Angular است که می‌توانید از آن، استفاده کنید:
...
قطعه کد زیر، یک نمونه از فایل `package.json` استاندارد برای برنامه‌های Angular است:
