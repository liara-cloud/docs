Original link: https://docs.liara.ir/paas/python/how-tos/deploy-app/
...
پروژه شما باید شامل فایل `requirements.txt` باشد؛ لیارا، در حین فرایند استقرار، به صورت خودکار این فایل را پیدا می‌کند و با اجرای دستور `pip install -r` تمامی ماژول‌های تعریف شده در این فایل را، برای‌تان نصب می‌کند.  
قطعه کد زیر، می‌تواند یک نمونه از فایل `requirements.txt` برای برنامه‌های Python باشد:
...
دستور اجرای برنامه در روش Liara Console، `python3 main.py` است و امکان تغییر آن در حال حاضر وجود ندارد.

## Liara CLI

پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام `liaraignore.` یا `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری‌های venv. و dist به همراه محتوای آن‌ها نیست؛ چرا که لیارا در حین استقرار برنامه، آن‌ها را برای شما می‌سازد؛ پس بایستی اسامی این دایرکتوری‌ها در یکی از دو فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های Python است که می‌توانید از آن، استفاده کنید:

[#### نمونه فایل `gitignore.` برای برنامه‌های Python](https://github.com/liara-cloud/gitignore-templates/blob/master/python/.gitignore)
...
در قطعه کد فوق، در فیلد `app` باید شناسه برنامه خود را به جای `my-python-app` وارد کنید.  
در فیلد `platform` باید نوع پلتفرم خود را که python است، مشخص کنید.  
اگر که برنامه‌تان وب‌سرور است و در پورت خاصی (به جز ۳۰۰۰) به درخواست کاربران گوش می‌دهد، باید در فیلد `port`، پورت برنامه را مشخص کنید.  
البته اگر که برنامه‌تان نیازی به پورت ندارد، می‌توانید این فیلد را از فایل `liara.json`، پاک کنید.
...
پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری‌های venv. و dist به همراه محتوای آن‌ها نیست؛ چرا که لیارا در حین استقرار برنامه، آن‌ها را برای شما می‌سازد؛ پس بایستی اسامی این دایرکتوری‌ها در فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های Python است که می‌توانید از آن، استفاده کنید:
...
اگر از کتابخانه یا پکیج خاصی در پروژه خود، استفاده می‌کنید که به صورت built-in یا پیش‌فرض در نسخه پایتون مدنظرتان وجود ندارد،  
پروژه شما باید شامل فایل `requirements.txt` باشد؛ لیارا، در حین فرایند استقرار، به صورت خودکار این فایل را پیدا می‌کند و با اجرای دستور `pip install -r` تمامی ماژول‌های تعریف شده در این فایل را، برای‌تان نصب می‌کند.  
قطعه کد زیر، یک نمونه از فایل `requirements.txt` برای برنامه‌های Python است:
