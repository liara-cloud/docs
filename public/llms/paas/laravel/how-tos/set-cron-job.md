Original link: https://docs.liara.ir/paas/laravel/how-tos/set-cron-job/

# تنظیم Cron Job

Cron job یک وظیفه زمان‌بندی شده در سیستم‌عامل‌های Unix و Linux است که به کاربران اجازه می‌دهد تا اسکریپت‌ها یا دستورات را در فواصل زمانی منظم اجرا کنند. این ابزار به خصوص برای انجام وظایف دوره‌ای مثل پشتیبان‌گیری، ارسال ایمیل، اجرای اسکریپت‌های نگهداری سیستم، یا به‌روزرسانی اطلاعات مفید است.

## Liara Console

برای تنظیم یک Cron Job در Laravel کافیست تا

پس از آپلود پروژه خود در کنسول در بخش **تنظیمات پلتفرم**، Cron Jobهای خود را تعریف کنید:

![set cron jobs on liara](https://media.liara.ir/docs/laravel-cronjob.png)

## Liara CLI

برای تنظیم یک Cron Job در Laravel کافیست تا در مسیر اصلی پروژه، یک فایل به نام `liara.json` ایجاد کنید و Cron Jobهای مد نظر خود را در آرایه‌ای به نام `cron`، تنظیم و پیکربندی کنید.  
به عنوان مثال، فرض کنید که باید یک‌سری دستورات مشخص موجود در فایلی به نام `commands.php`، در یک زمان مشخص، اجرا شوند؛ برای این‌کار، بایستی قطعه کد زیر را در فایل `liara.json` قرار دهید:

```json
{
  "cron": [
    "0 1 * * * cd $ROOT && php artisan your:command >> /dev/null 2>&1"
  ]
}
```

در نهایت، کافیست تا برنامه‌تان را با دستور `liara deploy` در لیارا مستقر کنید تا Cron Jobها برای‌تان فعال شوند.

## فرمت زمان‌بندی Cron Job

در نظر داشته باشید که فرمت زمان‌بندی Cron به صورت زیر است:

```scss
* * * * *
│ │ │ │ │
│ │ │ │ └── روزهای هفته (۰-۷) (۰ و ۷ هر دو نشان‌دهنده یکشنبه هستند)
│ │ │ └──── ماه‌ها (۱-۱۲)
│ │ └────── روزهای ماه (۱-۳۱)
│ └──────── ساعت‌ها (۰-۲۳)
└────────── دقیقه‌ها (۰-۵۹)
```

> با کمک [این](https://crontab.guru/) وب‌سایت، می‌توانید زمان دلخواه خود را، بسازید.

پس از تنظیم cron jobها و استقرار مجدد برنامه، می‌توانید لیست آن‌ها را در قسمت **تنظیمات برنامه** خود، مشاهده بفرمایید:

![see cron jobs on liara](https://media.liara.ir/docs/see-cron-jobs.png)

## all links

[All links of docs](https://docs.liara.ir/all-links-llms.txt)
