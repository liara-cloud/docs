Original link: https://docs.liara.ir/paas/dotnet/quick-start/

# استقرار سریع برنامه‌های NET.

## Liara Console

[Video link](https://media.liara.ir/dotnet/dotnet-desktop.mp4)
...
۳. ایجاد برنامه  
برنامه NET. خود را با شناسه، [شبکه خصوصی](../details/private-network) و [منابع سخت‌افزاری و بسته امکانات](../details/plans/about) مدنظرتان ایجاد کنید.

۴. حذف فایل‌های اضافی  
پوشه‌ها و فایل‌های درون پروژه که قصد ندارید در لیارا آپلود شوند (به عنوان مثال پوشه‌های nupkg و debug) را پاک کنید.

۵. بررسی وجود فایل csproj.  
حتماً باید در مسیر اصلی پروژه، یک فایل با پسوند `csproj.` وجود داشته باشد؛  
...
[Video link](https://media.liara.ir/dotnet/dotnet-cli.mp4)
...
۴. ایجاد برنامه  
با اجرای دستور زیر، برنامه NET. خود را ایجاد کنید:
```bash
liara create
```

> اگر که از قبل برنامه خود را ایجاد کرده‌اید؛ نیازی به انجام این کار نیست.  
در ادامه اجرای این دستور، باید به ترتیب، شناسه، نوع پلتفرم (که باید بر روی dotnet تنظیم شود)، شبکه خصوصی و [منابع سخت‌افزاری و بسته امکانات برنامه](../details/plans/about)، مشخص شوند.
...
[Video link](https://media.liara.ir/docs/dotnet-github.mp4)
...
> لیارا با اجرای دستور `dotnet restore` تمامی ماژول‌های موردنیاز برنامه را نصب می‌کند. پس برای نصب آن‌ها، نیازی به انجام کار خاصی نیست.

> همچنین بخوانید: [استقرار قدم به قدم برنامه‌های NET. در لیارا](./how-tos/create-app)
