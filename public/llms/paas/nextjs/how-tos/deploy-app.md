Original link: https://docs.liara.ir/paas/nextjs/how-tos/deploy-app/
...
قطعه کد زیر، یک نمونه از فایل `package.json` استاندارد برای برنامه‌های NextJS است:
...
[#### نمونه فایل `gitignore.` برای برنامه‌های NextJS](https://github.com/liara-cloud/gitignore-templates/blob/master/nextjs/.gitignore)
...
در فیلد `platform` باید نوع پلتفرم خود را که NextJS است، مشخص کنید.
...
پس از [ساخت برنامه](../create-app)، باید در مسیر اصلی پروژه، یک فایل به نام `gitignore.` ایجاد کنید و درون آن، اسامی تمامی فایل‌ها یا پوشه‌هایی که قصد ندارید در لیارا آپلود شوند را، وارد کنید؛ به عنوان مثال، نیازی به آپلود دایرکتوری node_modules به همراه محتوای آن نیست؛ چرا که لیارا در حین استقرار برنامه، آن را برای شما می‌سازد؛ پس بایستی نام این دایرکتوری در فایل فوق، نوشته شود؛ قطعه کد قرار گرفته در لینک زیر، یک `gitignore.` عالی برای برنامه‌های NextJS است که می‌توانید از آن، استفاده کنید:
...
همچنین، بایستی در این فایل، در فیلدی به نام `port`، پورتی 
