...
export default () => (
  <Layout>
    ...
    <p>
      حال برای انتقال فایل‌ها به دسترسی FTP نیاز است. بنابراین برای ایجاد دسترسی
      FTP وارد بخش دیسک‌ها در برنامه‌ی موردنظرتان شده و در بخش دسترسی FTP، بر
      روی دکمه‌ی ایجاد دسترسی FTP کلیک کنید.
    </p>
    <ZoomableImage src="https://files.liara.ir/docs/wordpress/create-new-ftp-access.gif" />

    <p>
      پس از ایجاد دسترسی FTP باید با استفاده از این پروتکل‌، فایل‌های دانلود شده
      از هاستینگ را در دیسک برنامه آپلود کنید.
    </p>
    <ZoomableImage src="https://files.liara.ir/docs/wordpress/upload-zip-file-with-ftp.gif" />

    <Notice variant="info">
      برای کسب اطلاعات بیشتر می‌توانید به{" "}
      <a href="/storage/disks/ftp">مستندات دسترسی به دیسک‌ها با FTP</a> مراجعه
      کنید.
    </Notice>
    <p>
      در مرحله‌ی آخر باید فایل zip آپلود شده در مرحله‌ی قبل را با اجرای دستور{" "}
      <span className="code">unzip file-name.zip </span> از حالت فشرده خارج
      کنید.
    </p>
    ...
    <p>
      برای انتقال دیتابیس از cPanel باید درابتدا از اطلاعات دیتابیس خود با
      استفاده از phpMyAdmin یک Export بگیرید.
    </p>
    <ZoomableImage src="https://files.liara.ir/docs/wordpress/export-mysql-database-using-phpmyadmin.gif" />

    <p>
      سپس باید وارد دیتابیس خود در لیارا شده و در بخش نحوه‌ی اتصال، phpMyAdmin
      را برای دسترسی آسان به دیتابیس راه‌اندازی کنید.
    </p>
    ...
    <p>
      ...
      زیر را به فایل <span className="code">wp-config.php</span> اضافه کرده و
      مجدد برنامه را بررسی کنید. درصورتی که خطا رفع نشد، می‌توانید ازطریق{" "}
      <a href="https://console.liara.ir/tickets">تیکت</a> با پشتیبان‌های فنی در
      ارتباط باشید.
    </p>
