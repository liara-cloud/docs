...
export default () => (
  <Layout>
    <Head>
      <title>توضیحات و نکات تکمیلی در برنامه‌های Angular - لیارا</title>
    </Head>
...
location ~* \\.(?:css|js|otf|ttf|eot|woff|woff2)$ {
  ...
  add_header Cache-Control "public";
    ...
    <p>
      Mirror اختصاصی لیارا به‌منظور دانلود سریع‌تر پکیج‌ها در پلتفرم Angular
      به‌صورت پیش‌فرض فعال است اما شما می‌توانید با قرار دادن قطعه‌کد زیر در
      فایل <Link href="/app-deploy/angular/liarajson">liara.json</Link>، این
  ...
  "angular": {
    ...
    <p>
      Source Map در پلتفرم Angular لیارا به‌صورت پیش‌فرض غیرفعال است اما درصورت
      نیاز به فعال کردن این قابلیت می‌توانید قطعه‌کد زیر را در فایل{" "}
      <Link href="/app-deploy/angular/liarajson">liara.json</Link> قرار دهید:
    </p>
    <Highlight className="json">
      {`{
  "angular": {
