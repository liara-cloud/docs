...
export default () => (
    <Layout>
        ...
        <p>
            ...
            صورتی که قصد دارید نسخه دیگری را برای اجرای برنامه‌ی‌تان استفاده
            کنید می‌توانید داخل فایل <span className="code">liara.json</span>{' '}
            بخش زیر را اضافه کنید: (فایل زیر برای یک برنامه تستی در نظر گرفته
            شده است.)
        </p>
  ...
  "laravel": {
    "phpVersion": "8.0"
        ...
        <p>
            ...
            داخل ریشه‌ی برنامه‌ی‌تان می‌توانید تنظیمات PHP را شخصی‌سازی کنید.
            برای مثال، ممکن است بخواهید که حداکثر حجم مجاز برای آپلود فایل در
            سایت لاراولی‌تان را شخصی‌سازی کنید. پس لازم است که فایل
            <span className="code">liara_php.ini</span>
            را به برنامه‌ی‌تان اضافه کرده و محتویات آن را برابر تکه‌کد قرار
            دهید:
        </p>
        <pre>
            <code>
...
max_execution_time = 600`}
        ...
        <p>
            بعد از اضافه کردن این فایل، در استقرار بعدی برنامه شما با این
            تنظیمات شروع به کار خواهد کرد.
        </p>
        <Notice variant="info">
            ...
            <Link href="/app-features/file-system">ReadOnly</Link> نیست، اما
            محدودیت ۱۰۰ مگابایتی دارد. این محدودیت به‌این معناست که اگر کاربران
            شما فایل‌های بزرگ‌تر از ۱۰۰ مگابایت را بخواهند آپلود کنند، با خطا
            مواجه خواهند شد. برای رفع این محدودیت، می‌توانید از قابلیت{' '}
            <Link href="/app-deploy/laravel/disks">دیسک‌ها</Link> استفاده کنید و
            دیسکی با اندازه‌ی دلخواه‌تان بسازید و به‌پوشه‌ی
            <span className="code">/tmp</span>
            متصل کنید.
        </Notice>
        ...
        <p>
            ...
            فایل‌های SASS به CSS تبدیل می‌شوند و همین‌طور کدهای جاوا اسکریپت ES6
            به بالا به ES5 تبدیل شده و یک‌پارچه می‌شوند.
            <br />
            هر زمان که برنامه‌ی‌تان را روی لیارا مستقر می‌کنید، ما این دستور را
            برای‌تان اجرا می‌کنیم تا مطمئن شویم که فایل‌های CSS و JS شما به صورت
            صحیح در اختیار کاربران‌تان قرار گیرند.
        </p>
        <p>
            اما اگر از Laravel فقط برای ساخت یک API استفاده کرده‌اید و یا به طور
            کلی نیازی به این ندارید که لیارا پکیج‌های npm را برای‌تان نصب و
            فایل‌های CSS و JavaScript تان را build کند، می‌توانید در فایل{' '}
            <Link href="/app-deploy/laravel/liarajson">liara.json</Link> برنامه،
            یک فیلد با نام laravel و داخل آن یک فیلد با نام
  ...
  "laravel": {
    "buildAssets": false
        ...
        <p>
            ...
            لاراول، Supervisor نصب شده و شما با ایجاد یک فایل به نام{' '}
            <span className="code">supervisor.conf</span> در ریشه‌ی برنامه‌،
            می‌توانید تنظیمات صف‌های مختلف‌تان را در آن وارد کنید. و در نهایت با
            یک‌بار دیپلوی‌کردن، صف‌های شما شروع به کار خواهند کرد.
        </p>
        <p>
            Supervisor برنامه‌ی بسیار مفیدی است که سعی می‌کند صف‌های شما را
            همیشه در حال اجرا نگه‌دارد. اگر به هر دلیلی صف‌های‌تان به خطا بخورند
            و خاموش شوند، Supervisor آن‌ها را دوباره ایجاد و فعال می‌کند.
        </p>
        <p>
            از بخش <Link href="/apps/console">خط فرمان (کنسول)</Link>{' '}
            برنامه‌ی‌تان هم می‌توانید با supervisorctl کار کنید و وضعیت
...
stdout_logfile=/tmp/laravel-worker.log`}
        ...
        <p>
            ...
            <span className="code">storage</span>، طبق مستندات لاراول باید این
            پوشه به پوشه‌ی <span className="code">public</span> لینک شود. لیارا
            به‌صورت خودکار، در زمان استقرار، دستور
        ...
        <p>
            برای رفع خطای CORS در برنامه‌های Laravel، دو راه حل مختلف پیش روی
            شما قرار دارد. در راه حل اول می‌توانید پکیج{' '}
        ...
        <p>
            ...
            <span className="code">public/.htaccess</span> پروژه‌ی خود اضافه
            کنید:
        </p>
...
<IfModule mod_rewrite.c>
    ...
    RewriteRule ^ index.php [L]
        ...
        <p>
            ...
            <span className="code">php artisan config:cache</span> است که وظیفه
            کش کردن کانفیگ‌فایل‌ها را به عهده دارد و دستور دوم{' '}
            <span className="code">php artisan route:cache</span> است که وظیفه
            کش کردن فایل‌های route را به عهده دارد. برای این که این دستورات به
            صورت خودکار توسط لیارا در هر استقرار اجرا شود کافیست که در فایل{' '}
  ...
  "laravel": {
    ...
    "routeCache": true
        ...
        <Notice variant="info">
            ...
            <span className="code">"configCache": true</span> استفاده کنید و
            فیلد دیگر را قرار ندهید.
        </Notice>
        <Notice variant="danger">
            ...
            ، استفاده از دستور php artisan route:cache تنها زمانی امکان‌پذیر است
            که شما از Closure ها برای تعریف route ها استفاده نکرده باشید و فقط
            از Controller استفاده کرده باشید.
        </Notice>
        ...
        <p>
            ...
            های لیارا هدایت می‌شوند. برای این که در برنامه‌ی‌تان بتوانید به
            آی‌پی واقعی کاربر دسترسی داشته باشید و یا این که از قابلیت Signed
            URL های Laravel استفاده کنید، لازم است که تغییراتی را در فایل
...
class TrustProxies extends Middleware
{
    ...
    protected $headers = Request::HEADER_X_FORWARDED_ALL;
        ...
        <p>
            لاگ‌ها وظیفه دارند اتفاقات رخ داده در نرم‌افزار مثل error ها یا
            exception ها و حتی اطلاعاتی که خود برنامه‌نویس به دلخواه خود در
            بخش‌های مختلف نرم‌افزار درنظرگرفته را، ثبت کنند. Laravel روش‌های
            مختلفی برای لاگ‌ کردن دارد که اصطلاحا به آن‌ها Channel Drivers
            میگوید. برای آشنایی بیشتر به{' '}
        ...
        <p>
            برای نمونه اگر قصد دارید Log ها را روی Console بریزید، تا در پنل
            لاگ‌های داشبورد لیارا قابل مشاهده باشد می‌توانید از درایور
            <span className="code">errorlog</span>
            استفاده کنید. اگر قصد دارید Log ها را داخل یک فایل نگه‌دارید از
            درایور <span className="code">single</span> استفاده کنید. اگر قصد
            دارید Log ها را بر اساس روز در فایل‌های مجزایی نگه‌داری کنید تا حجم
            هر فایل برای بررسی زیاد نشود می‌توانید از درایور{' '}
            <span className="code">daily</span> استفاده کنید. تصمیم نحوه‌
            نگه‌داری Log ها با شماست و در صورتی که قصد تغییر آن را دارید به
            راحتی از طریق بخش تنظیمات برنامه می‌توانید نام درایور موردنظر خودتون
            رو به env ها اضافه کنید:
        </p>{' '}
        <Highlight className="json">{`LOG_CHANNEL=daily`}</Highlight>
        <p>یا</p>
        <Highlight className="json">{`LOG_CHANNEL=errorlog`}</Highlight>
        <Notice variant="info">
            Laravel قابلیت خوبی برای استفاده همزمان از چند درایور دارد. اگر شما
            از درایور <span className="code">stack</span> اسفتاده کنید می‌توانید
            همزمان Log ها را به چند درایور ارسال کنید. مثلا هم داخل فایل
            نگه‌داری شود و هم به Console ریخته شود تا در پنل لیارا قابل مشاهده
            باشد. درباره این درایور در{' '}
            <a
        ...
        <Notice variant="info">
            در برنامه تستی آموزش راه‌اندازی Laravel در لیارا، از درایور stack
            برای لاگ همزمان داخل فایل و کنسول استفاده شده است. می‌توانید{' '}
            <a
        ...
        <p>
            برای فعال‌سازی Caching در برنامه‌های Laravel باید تنظیمات مربوطه را
            در فایل <span className="code">public/.htaccess</span> برنامه وارد
            کنید:
        </p>
        <pre>
            <code>
 ...
 <filesmatch "\\.(js)$">
  Header set Cache-Control "max-age=216000, private"
    ...
    mix.version();
        ...
        <p>
            درنهایت باید فایل‌های Asset را مانند مثال زیر در برنامه‌ی خود
            فراخوانی کنید:
        </p>
        <pre>
        ...
        <p>
            حال با اجرای دستور <span className="code">liara deploy</span>،
            فرایند استقرار آغاز خواهد شد.
        </p>
