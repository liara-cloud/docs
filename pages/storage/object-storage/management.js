...
export default () => (
  <Layout>
    ...
    <p>
      هر باکت در سرویس فایل دارای سطح دسترسی میباشد. برای مثال، میتوانید برای
      ...
      پیشوندی مثل videos شروع میشوند را عمومی قرار دهید و به فایل‌هایی که با
      پیشوند test_videos شروع میشوند دسترسی خصوصی دهید. در نهایت، چگونگی سطح‌
      دسترسی‌ها وابسته به تصمیم شما برای ساختار مدیریت فایل‌هایتان میباشد.
    </p>
    <p>به صورت کلی در سرویس‌فایل شما میتوانید این دسترسی‌ها را ایجاد کنید:</p>
    <ul>
      <li>
        <p>
          <b>
          ...
          حالت پیشفرض None میباشد. در این حالت تمام ارتباطات برای دانلود یا
          آپلود فایل به باکت‌ها از طریق access key و secret key میباشد.
        </p>
      </li>
    ...
    <p>
      برای تغییر این سطح دسترسی‌ها میتوانید از دو راه داشبورد گرافیکی یا CLI
      استفاده کنید. برای تغییر سطح دسترسی‌ها در حالت گرافیکی میتوانید وارد
    ...
    <p>
      برای نمونه به تمام فایل‌های داخل باکت public-files بدین‌صورت میتوانید
      دسترسی عمومی برای خواندن بدهید. کاراکتر <b>*</b> به صورت Wild card عمل
      میکند و تنظیمات را روی همه فایل‌ها اعمال میکند.
    </p>
    <ZoomableImage src="/static/update-policy.png" />
    <p>
      همچنین اگر میخواهید فقط به فایل‌هایی که با پیشوند <b>/public</b> شروع
      میشوند، دسترسی عمومی خواندن بدهید بدین صورت انجام دهید:
    </p>
    <ZoomableImage src="/static/update-policy-public.png" />
    <p>
      در صورتی که قصد دارید از طریق CLI سطح دسترسی‌ها را تغییر دهید میتوانید از{" "}
      <a
      ...
      استفاده کنید. مثلا میتوانید به صورت زیر به باکت <b>users</b> دسترسی عمومی
      خواندن بدهید:
    </p>
    <pre>
    ...
    <p>
      برای دیدن اطلاعات بیشتر درباره MinIO CLI میتوانید به{" "}
      <a
    ...
    <p>
      ...
      شما به شکل زیر در دسترس خواهد بود: (این آدرس برای مثال میباشد.)
    </p>
    <pre>
      <code>
    ...
    <p>
      اگر از دامنه اختصاصی استفاده کرده ‌باشید به جای آدرس پیشفرض لیارا میتوانید
      دامنه‌ی خودتان را قرار دهید.
    </p>
    ...
    <p>
      ...
      میشود). در صورتی که قصد دارید دامنه‌ شخصی‌ خودتان را برای سرویس فایل
