...
<Layout>
<Head>
<title>مستندات کار با AI SDK در VueJS (Nuxt) - لیارا</title>
<meta property="og:title" content="مستندات خدمات رایانش ابری لیارا" />
<meta property="og:description" content="مستندات مربوط به استفاده از AI SDK در VueJS (Nuxt) محصول هوش مصنوعی لیارا"  />
</Head>
...
برای شروع، یک برنامه‌ی جدید <a href="/paas/nodejs/related-apps/nuxtjs/" className="text-[#2196f3]">Nuxt</a>، ایجاد کنید. دستور زیر یک دایرکتوری جدید به نام <Important>my-ai-app</Important> ایجاد می‌کند و درون آن، یک پروژه پایه Nuxt می‌سازد:
...
<li><Important>ai-sdk/vue@</Important>: توابع مربوط به Vue.js برای کار با SDK</li>
...
در قطعه کد فوق، به‌جای <Important>xxxxxxxxx</Important>، مقادیر واقعی خود را، قرار دهید؛ در ادامه، متغیرهای تعریف‌شده 
را مانند قطعه کد زیر در فایل <Important>nuxt.config.ts</Important>، وارد کنید:
...
در مسیر <Important>server/api/chat.ts</Important>، یک API Route بسازید و قطعه کد زیر را درون آن، قرار دهید:
...
۱. یک Event Handler تعریف شده است که پیام‌ها را از بدنه‌ی request استخراج می‌کند. متغیر <Important>messages</Important> شامل سابقه‌ی گفتگو میان شما و چت‌بات است و context لازم را برای چت‌بات فراهم می‌کند تا پاسخ بعدی را تولید کند.
...
حالا که یک API route دارید که می‌تواند با یک LLM ارتباط برقرار کند، زمان آن رسیده که رابط کاربری (frontend) خود را راه‌اندازی کنید. پکیج UI در AI SDK، پیچیدگی‌های مربوط به رابط چت را در یک هوک به نام <Important>useChat</Important> ساده کرده است.
در فایل  <Important>pages/index.vue</Important>، قطعه کد زیر را قرار دهید تا یک لیست از پیام‌های چت، نمایش داده شود و امکان ارسال پیام کاربر، فراهم گردد:
...
<Alert variant="warning">
<p>
اگر پروژه شما شامل <Important>app.vue</Important> به‌جای <Important>pages/index.vue</Important> است، تنها کافیست تا فایل <Important>app.vue</Important> را پاک کنید و یک <Important>pages/index.vue</Important> جدید با قطعه کد فوق ایجاد کنید.
</p>
</Alert>
 
این صفحه از هوک <Important>useChat</Important> استفاده می‌کند که به‌صورت پیش‌فرض، از API Route که قبل‌تر ایجاد کرده بودید، استفاده خواهد کرد.
...
<div className="h-2" />   
<img src="https://media.liara.ir/ai/ai-sdk/nuxt/working-with-chat-bot.png" alt="work with chatbot in VueJS (Nuxt)"/>
<div className="h-4" />
...
<div className="h-2" />   
مانند قطعه کد زیر، به فایل <Important>server/api/chat.ts</Important> خود، Tool مربوط به وضعیت آب‌وهوا را اضافه کنید:
...
برای به‌روزرسانی رابط کاربری و نمایش <Important>tool invocation</Important>، باید فایل <Important>pages/index.vue</Important> را تغییر دهید تا پیام‌هایی که شامل Tool هستند نیز، به درستی نمایش داده شوند.
...
<div className="h-2" />   
<img src="https://media.liara.ir/ai/ai-sdk/nuxt/working-with-tools-in-chatbots.png" alt="work with chatbot in vuejs (nuxt) with tools"/>
<div className="h-4" />
<hr className="mb-2" />
...
فایل <Important>pages/index.vue</Important> خود را ویرایش کرده تا گزینه‌ی <Important>maxSteps</Important> را به آن، اضافه کنید:
...
<div className="h-2" />   
<img src="https://media.liara.ir/ai/ai-sdk/nuxt/chatbots-tools-and-maxsteps.png" alt="work with chatbot in vuejs (nuxt) with tools and maxsteps"/>
<div className="h-4" />
...
Tool جدید برای تبدیل دما از فارنهایت به سلسیوس را به فایل <Important>server/api/chat.ts</Important> خود، مانند شکل زیر، اضافه کنید:
...
<div className="h-2" />   
<img src="https://media.liara.ir/ai/ai-sdk/nuxt/chatbot-with-two-tools-and-maxsteps.png" alt="work with chatbot in nuxt with two tools and maxsteps on 5"/>
<div className="h-4" />
...
<p>
پروژه نهایی مورد بررسی در این آموزش، در <a href="https://github.com/liara-cloud/ai-sdk-examples/tree/master/Vue-Nuxt-ChatBot" className="text-[#2196f3]">گیت‌هاب لیارا</a> موجود است که می‌توانید از آن، استفاده کنید.
</p>
</Alert>
