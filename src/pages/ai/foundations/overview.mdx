...
به‌عنوان مثال، شما می‌توانید برای برقراری ارتباط با مدل‌های هوش مصنوعی، مانند شکل زیر از <Important>AI SDK</Important> که یکی از فوق‌العاده‌ترین SDKهای ساخته شده برای هوش مصنوعی است؛ استفاده کنید:
...
<HighlightTabs
  tabs={[
    {
      label: "xAI",
      icon: <PlatformIcon platform={document.documentElement.classList.contains("dark")?"https://ofs-liara.storage.c2.liara.space/logos/ai/xai-dark.svg":"https://ofs-liara.storage.c2.liara.space/logos/ai/xai-light.svg"} style={{ width: 18, height: 18,color:"#ccc" }} />,
      language: "javascript",
...
const { text } = await generateText({
  ...
  prompt: "What is love?"
    ...
    },
    {
      ...
      icon: <GoTelescope />,
      language: "javascript",
      code: `const response = await googleGenerate({
  prompt: "What is love?",
  model: "gemini-1.5"
})`,
    },
    {
      ...
      icon: <GoTelescope />,
      language: "javascript",
      code: `const answer = await myModel.generate({
  prompt: "What is love?"
})`,
    },
  ]}
