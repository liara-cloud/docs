...
<ul>
  ...
  <li><Important>text</Important>: متن مستقیم تولید شده توسط مدل (در اینجا خالی است چون از ابزار استفاده شده)</li>
  <li><Important>finishReason</Important>: دلیلی که مدل را متوقف کرده (اینجا "tool-calls" یعنی به‌خاطر فراخوانی ابزار)</li>
  <li><Important>isContinued</Important>: مشخص می‌کند آیا پاسخ در پیام‌های بعدی ادامه دارد یا خیر</li>
  <li><Important>toolCalls</Important>: لیستی از فراخوانی‌هایی است که مدل برای ابزارها انجام داده</li>
  <li><Important>toolResults</Important>: پاسخ‌هایی هستند که ابزارها به فراخوانی‌های مدل داده‌اند</li>
  <li><Important>usage</Important>: میزان استفاده از توکن‌ها برای این تعامل را نشان می‌دهد</li>
  <li><Important>request</Important>: اطلاعات مربوط به درخواستی که به API ارسال شده را شامل می‌شود</li>
  <li><Important>response</Important>: پاسخ کامل دریافت‌شده از API، شامل بدنه، هدرها و پیام‌ها</li>
  <li><Important>providerMetadata</Important>: متادیتای مرتبط با ارائه‌دهنده مدل است که به صورت آزمایشی استفاده می‌شود</li>
  <li><Important>experimental_providerMetadata</Important>: مشابه providerMetadata، اطلاعات اضافی آزمایشی مدل را نشان می‌دهد</li>
</ul>
...
<div className="h-2" />
<ul>
  <li><Important>type</Important>: نوع فراخوانی ابزار را مشخص می‌کند (در اینجا "tool-call")</li>
  <li><Important>toolCallId</Important>: شناسه یکتای این فراخوانی ابزار</li>
  <li><Important>toolName</Important>: نام ابزاری که فراخوانی شده (مثل getCurrentWeather)</li>
  <li><Important>args</Important>: آرگومان‌هایی که به ابزار ارسال شده‌اند</li>
</ul>
...
<div className="h-2" />
<ul>
  <li><Important>locations</Important>: (اشتباه تایپی است، درست: <Important>location</Important>): شهری که برای آن آب‌وهوا درخواست شده</li>
...
<div className="h-2" />
<ul>
  <li><Important>type</Important>: نوع نتیجه ابزاری را مشخص می‌کند (اینجا "tool-result")</li>
  <li><Important>toolCallId</Important>: شناسه مربوط به toolCallی که نتیجه‌اش این است</li>
  <li><Important>toolName</Important>: نام ابزاری که این نتیجه را برگردانده</li>
...
<div className="h-2" />
<ul>
  <li><Important>location</Important>: شهر و کشور مقصد درخواست آب‌وهوا</li>
...
<div className="h-2" />
<ul>
  <li><Important>body</Important>: محتوای کامل درخواست ارسالی به API به صورت رشته JSON</li>
</ul>
...
<ul>
  ...
  <li><Important>content-length</Important>: طول محتویات پاسخ در بایت</li>
  <li><Important>content-type</Important>: نوع محتوای پاسخ (JSON)</li>
  <li><Important>date</Important>: تاریخ و زمان ارسال پاسخ</li>
  <li><Important>server</Important>: اطلاعات درباره سرور پاسخ‌دهنده (مثلاً nginx)</li>
</ul>
...
<div className="h-2" />
<ul>
  <li><Important>id</Important>: شناسه یکتای تعامل تولید شده توسط API</li>
...
<ul>
  ...
  <li><Important>refusal</Important>: در صورت امتناع از پاسخ، دلیل آن</li>
  <li><Important>reasoning</Important>: توضیح منطق پشت تصمیم (در اینجا null)</li>
  <li><Important>tool_calls</Important>: لیست فراخوانی‌های ابزاری انجام شده توسط مدل</li>
</ul>
...
<div className="h-2" />
<ul>
  <li><Important>index</Important>: ترتیب این tool_call در لیست</li>
  <li><Important>id</Important>: شناسه یکتای این فراخوان ابزار</li>
...
<ul>
  ...
  <li><Important>prompt_tokens_details.cached_tokens</Important>: تعداد توکن‌هایی که از کش استفاده شده‌اند (در اینجا صفر)</li>
...
<ul>
  ...
  <li><Important>cachedPromptTokens</Important>: توکن‌هایی که از کش استفاده شده‌اند</li>
</ul>
