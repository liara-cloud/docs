...
`}
    ...
    <>
      ...
      <ul>
        ...
        <li><Important>execute</Important>: یک تابع async اختیاری که با آرگومان‌های داده شده توسط فراخوانی tool، صدا زده می‌شود. این خصیصه، یک مقدار از نوع <Important>RESULT</Important> ایجاد می‌کند. </li>
      </ul>
      <div className="h-4" />
      <p>
        پارامتر <Important>tools</Important> در <Important>generateText</Important> و <Important>streamText</Important>، یک آبجکت است که باید در آن، اسم tool را به عنوان کلید تعریف کرده و خود tool را به‌عنوان مقدار، به کلید تعریف شده، بدهید: 
      </p>
