...
<div className="h-2" />
<ul className="pr-4">
  {models.deepseek.map((item, index) => (
    <li
      key={index}
      style={{
...
پس از <a href="/ai/quick-start" className="text-[#2196f3] " >ایجاد سرویس هوش مصنوعی</a> و دریافت <Important>baseUrl</Important> و <a href="/ai/details/keys/#create" className="text-[#2196f3]" >ساخت کلید</a>، می‌توانید از مدل‌های DeepSeek استفاده کنید.
