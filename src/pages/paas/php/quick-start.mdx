...
<Layout>
<Head>
<title>مستندات استقرار سریع برنامه‌های PHP - لیارا</title>
<meta property="og:title" content="مستندات خدمات رایانش ابری لیارا" />
<meta property="og:description" content="مستندات مربوط به استقرار سریع برنامه‌های PHP در لیارا"  />
</Head>

# استقرار سریع برنامه‌های PHP
<hr className="mb-2" />
...
<Tabs 
  ...
  content={[
   <>
  <video
  src="https://media.liara.ir/php/php-desktop.mp4"
  controls="controls"
  ...
  <Step  steps={[
{
  ...
  <>
    <h3>ایجاد برنامه</h3>
    <p>برنامه PHP خود را با شناسه، <Link className="text-[#2196f3] " href="../details/private-network">شبکه خصوصی</Link> و <Link className="text-[#2196f3] " href="../details/plans/about">منابع سخت‌افزاری و بسته امکانات</Link> مدنظرتان ایجاد کنید.</p>
  </>
  )
  },
  {
  ...
  <>
    ...
    <p>پوشه‌ها و فایل‌های درون پروژه که قصد ندارید در لیارا آپلود شوند (به عنوان مثال پوشه‌های bin و vendor) را پاک کنید.</p>  
  </>
  )
  },
  {
  ...
  content: (
  <>
    <h3>تنظیم فایل‌های composer</h3>
    <p>اگر که در پروژه PHP خود از composer برای نصب پکیج خاصی استفاده کرده باشید؛ حتماً باید در مسیر اصلی پروژه، فایل <Important>composer.json</Important> وجود داشته باشد؛ همچنین بهتر است
    برای جلوگیری از خطاهای احتمالی استقرار، فایل <Important>composer.lock</Important> موجود در مسیر اصلی پروژه خود را، پاک کنید.
    </p>
  </>
  )
  },
  ...
  <>
  <video
  src="https://media.liara.ir/php/php-cli.mp4"
  controls="controls"
  className="block w-full"
  width="100%"
  ></video>
  ...
  <Step  steps={[
{
  ...
  <>
    <h3>ایجاد برنامه</h3>
    <p>با اجرای دستور زیر، برنامه PHP خود را ایجاد کنید:</p>
    <div dir='ltr'>
    <Highlight className="bash">
      {`liara create`}
    </Highlight>
    ...
    <p>در ادامه اجرای این دستور، باید به ترتیب، شناسه، نوع پلتفرم (که باید بر روی php تنظیم شود)، شبکه خصوصی و <Link className="text-[#2196f3] " href="../details/plans/about">منابع سخت‌افزاری و بسته امکانات برنامه</Link>، مشخص شوند.</p>
  </>
  )
  },
  {
...
/bin`}
  ...
  <>
    ...
    <div dir='ltr'>
    <Highlight className="bash">
      {`liara deploy --app=myapp --platform=php`}
    </Highlight>
    </div>
    <div className="h-2" />
    <p>
    ...
    <div dir='ltr'>
    <Highlight className="bash">
      {`liara deploy --app=myapp --platform=php --team-id=your-unique-team-id`}
    </Highlight>
    </div>
  </>
  )
  ...
  <>
    <video
    src="https://media.liara.ir/docs/php-github.mp4"
    controls="controls"
    className="block w-full"
    width="100%"
    ></video>
    ...
    <Step  steps={[
{
  ...
  content: (
   <>
    <h3>تنظیم فایل‌های composer</h3>
    <p>اگر که در پروژه PHP خود از composer برای نصب پکیج خاصی استفاده کرده باشید؛ حتماً باید در مسیر اصلی پروژه، فایل <Important>composer.json</Important> وجود داشته باشد؛ همچنین بهتر است
    برای جلوگیری از خطاهای احتمالی استقرار، فایل <Important>composer.lock</Important> موجود در مسیر اصلی پروژه خود را (در صورت وجود)، پاک کنید.
    </p>
  </>
  )
  },
...
در صورتی که از قبل، این کار را انجام داده‌اید یا قصد دارید از ریپازیتوری‌های فعلی خود استفاده کنید؛ از این مرحله بگذرید.
  ...
  <>
    ...
    <div className="h-1" />
    <img src="https://media.liara.ir/docs/connect-to-github-on-liara.png" alt="connect to github on liara account" />
    <div className="h-1" />
    <p> 
      پس از 
    ...
    <div className="h-1" />
    <img src="https://media.liara.ir/docs/added-github-account-on-liara.png" alt="added github account on liara" />
    <div className="h-1" />
...
<Alert variant="info">
<p>
در صورت وجود فایل <Important>composer.json</Important> لیارا با اجرای دستور <Important>composer install</Important> تمامی ماژول‌های موجود در این فایل را نصب می‌کند. پس حتماً اسامی آن‌ها باید در این فایل باشد.
</p>
</Alert>

<Alert variant="success">
<p>
همچنین بخوانید: <Link className="text-[#2196f3] " href="./how-tos/create-app">استقرار قدم به قدم برنامه‌های PHP در لیارا</Link>
</p>
</Alert>
</Layout>