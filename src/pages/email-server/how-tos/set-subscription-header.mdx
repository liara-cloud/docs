...
<div className="h-4" />
<img src="https://files.liara.ir/liara/email-server/subscription-header.png" alt="mail_with_subscriptions" />
<div className="h-4" />
...
Mail::raw('Hello, this is a test email sent from Laravel.', function ($message) {
    $message->to('someguy@example.com')
        ...
        ->withSwiftMessage(function ($swiftMessage) {
            ...
            $headers->addTextHeader('List-Subscribe', '<http://www.example.com/subscribe>');
            $headers->addTextHeader('List-Unsubscribe', '<http://www.example.com/unsubscribe>');
        });
});
`}
            </Highlight>
...
try {
    ...
    $mail->addCustomHeader('List-Subscribe', '<http://www.example.com/subscribe>');
    $mail->addCustomHeader('List-Unsubscribe', '<http://www.example.com/unsubscribe>');
...
def send_email():
    ...
    message["List-Subscribe"] = "<http://www.example.com/subscribe>"
    message["List-Unsubscribe"] = "<http://www.example.com/unsubscribe>"
